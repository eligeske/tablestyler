...
$(document).ready(function(){
 ...
 var set = {
		...
		makeCss : function(){
			...
			this.css += '.datagrid table tbody td:first-child { border: none; }';
			if(this.tfoot){
		...
		makeTR:function($alt, $rows){
			$cls = ' class="alt"';
			$f = false; $tr = "";
			for(i = 1; i <= $rows; i++){
				$tr += "<tr"+(($f && $alt)?$cls:"")+"><td>Row "+i+"</td><td>data "+i+"</td><td>data "+i+"</td><td>data "+i+"</td></tr>\n";
				$f = ($f)?false:true;
			}
			return $tr;
		},
		makeOutputHtml:function(){
			...
			if(this.thead){
				$output += '<thead><tr><th>Title</th><th>Column 1</th><th>Column 2</th><th>Column 3</th></tr></thead>\n'; 
			}
			if(this.tfoot){
				$output += '<tfoot><tr><td colspan="4">';
				if(this.tfootPaging){
			...
			$output += '<tbody>';
			$output += this.makeTR(this.tbodyAltRows, 5);	
			$output += '</tbody>\n';
			$output += '</table></div>';
			return $output;
		},
